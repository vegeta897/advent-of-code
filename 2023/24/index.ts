...
// https://stackoverflow.com/a/54866504/2612679
...
export const getPart2Answer: Answer = (input: string): string | number => {
	...
	for (let vxi = 0; vxi < possibleVXs.length; vxi++) {
		const vx = possibleVXs[vxi]
		for (const vy of possibleRockVs[1]) {
			for (const vz of possibleRockVs[2]) {
				...
				if (axisIntersectCount === 3) {
					...
					for (let i = 0; i < hailIntersectionTimes.length; i++) {
						const [hailIndex, time] = hailIntersectionTimes[i]
...
export const part2Examples: Example[] = [[part1Examples[0][0], '47']]
